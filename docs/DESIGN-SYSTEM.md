...
A public knowledge node on Holonet: cyber-noir, precise, quiet. The reader comes for useful cybersecurity reasoning. Use the operations-centre character in the masthead, short section labels and occasional dry phrasing; keep article titles and prose descriptive.

The homepage opens with `HOLOCRON // PUBLIC SECURITY ARCHIVE` and `HOLONET KNOWLEDGE NODE`, followed by “Practical cybersecurity knowledge by Darkcybe.” Native cards expose the five reader journeys. Start Here explains the evidence model. Default GitBook content width keeps prose readable. The eight initial reader pages establish the publication's intent; they are not a claim that the historical corpus has been refreshed.
...
- Holocron is the bounded public knowledge repository and publication. Holonet is the information and automation plane; Death Star is the laboratory and experimental environment.
- Darkcybe is the author/operator and publisher. Use plain first-person statements only when Darkcybe's supplied experience supports them.
...
The repository homepage uses a portable Markdown blockquote for its K-2AI credit, readable on GitHub and GitBook. Inside articles, justified K-2AI callouts may use an `info` hint with the full editorial-system label and a short, specific contribution. Use them occasionally, not on every section. Do not use them to manufacture a second authorial voice or repeat the article summary.
...
Anything committed here is assumed public, including authoring templates and repository machinery outside reader navigation. Private Obsidian content, customer/case data, HOLONET secrets/runtime material and unsanitised evidence must never be staged here.

The stdlib validator checks the tracked Git index for environment-secret and credential filenames, key material, database files, packet captures, raw logs/dumps and archive bundles. No archive workflow exception is currently approved. It scans text, including examples and comments, for private-key headers and selected GitHub, AWS and Slack credential markers. Use visibly redacted placeholders in articles. It rejects tracked symlinks and submodules rather than following them.

Stage only intended public changes before running validation. This gate does not inspect Git history, untracked files, encoded secrets or every possible credential format. It is not generic DLP: ordinary security terminology and private lab addresses are legitimate article content. CI may prevent obvious mistakes; it does not replace Darkcybe's PR review.
...
Local validation checks structure and obvious publication-safety mistakes; it does not establish sanitisation or technical accuracy. Later render review must inspect card wrapping, navigation grouping, hint appearance, tables, contrast, mobile layout and the exclusion of authoring files from the reader tree. The A02 review question is: **Does Holocron express Darkcybe’s public knowledge publication?**
